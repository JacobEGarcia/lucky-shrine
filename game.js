...
const lintelCrust = []; let crustSheds = 0, prevCrustBreeze = null; // v122: the gates grow a crust through the cold, the gusts crack it off as plume
...
const plumeMotes = []; let plumeCount = 0, nextPlumeT = 0; // v105: snow smoking off the torii lintel drifts
...
function tick() {
  ...
  if (SEASON === 'snow') {
    ...
    if (traxLive) {
      ...
      for (let i = 0; i < foxTracks.length; i++) {
        ...
        t.m.material.opacity = t.op * t.reveal * (1 - 0.35 * ei) * Math.max(0, 1 - 1.1 * humpAcc);
        t.m.scale.set(0.75 * (1 - 0.17 * ei), 1.6 * (1 + 0.31 * ei), 1); // smeared downwind, crisp rim gone
  ...
  if (SEASON === 'snow') {
    ...
    if (prevCrustBreeze === null) prevCrustBreeze = furinBreeze; // first tick: no edge, just learn the air
      ...
      for (const c of lintelCrust) c.shed = 1;
      crustSheds++;
    }
  ...
  if (SEASON === 'snow' && furinBreeze > 0.85 && T > nextPlumeT) {
    nextPlumeT = T + 0.035;
    const pg = Math.random() < 0.55 ? plumeGates[0] : plumeGates[1];
  ...
  for (let i = 0; i < tiedStrips.length; i++) {
    ...
    st.rotation.x = (Math.sin(T * 1.8 + i * 0.9) * 0.16 * sw * (1 + paperLoad * 0.1) + Math.sin(T * 6.5 + i * 1.1) * 1.0 * sg * (1 + paperLoad * 0.1)) * (1 - 0.55 * stripTight) + 1.15 * stripTight; // v110: flutter gives way to streaming downwind
    st.rotation.z = st.userData.baseZ + ((tiedStrips.length >= 8 ? Math.sin(T * 7 + i * 1.7) * 0.05 * sw : 0) + Math.sin(T * 5.1 + i * 0.7) * 0.5 * sg) * (1 - 0.55 * stripTight);

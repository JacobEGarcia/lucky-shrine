...
if (SEASON === 'snow') {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    const mat = new THREE.MeshStandardMaterial({ color: 0xd6e0e8, roughness: 0.5, transparent: true, opacity: 0.14 + 0.5 * (1 - i / steps), emissive: 0x8fa5b5, emissiveIntensity: 0.3 });
  ...
  // honden where the cat curled up and the snow kept falling - a small cat-shaped hump
  {
    ...
    for (let i = 0; i < 17; i++) {
      ...
      const mat = new THREE.MeshStandardMaterial({ color: 0xd6e0e8, roughness: 0.5, transparent: true, opacity: 0.3 + 0.2 * (i / 17), emissive: 0x8fa5b5, emissiveIntensity: 0.3 });
      const pr = new THREE.Mesh(pawGeo, mat);
    ...
    for (const [gm, gy] of [[bed, 0.3], [body, 0.22], [head, 0.16], [tail, 0.14]]) humpGrowMeshes.push({ m: gm, sx: gm.scale.x, sy: gm.scale.y, sz: gm.scale.z, gy });
...
function applyAccum() {
  ...
  for (const g of humpGrowMeshes) {
    if (g.gy >= 0) g.m.scale.set(g.sx * (1 + a * g.gy * 0.6), g.sy * (1 + a * g.gy), g.sz * (1 + a * g.gy * 0.6));
...
function updateAccum(dt) {
  if (SEASON !== 'snow' || !started || humpAcc >= 1) return;
...
function fireBreath() {
  breathCount++;
  const mz = foxMuzzles[breathCount % foxMuzzles.length];
  ...
  p.position.copy(mz);
  p.userData = { t: 0, dir: new THREE.Vector3((Math.random() - 0.5) * 0.02, 0.02, 0.1) };

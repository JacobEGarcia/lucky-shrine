...
const boxFrost = []; // v144: the old coins in the box, frosted through the night - wiped where a fresh koban lands
const woodRBase = new THREE.Color(0x8e2f22), woodRCold = new THREE.Color(0x63463c); // v146: the vermilion's warm and cold-dulled poles
...
let iceTicks = 0, nextIceTickT = 0; // v142: hard gusts set the eave icicles ticking - a faint gamelan
...
// ================= SHRINE (honden) =================
{
  ...
  if (SEASON === 'snow') {
    ...
    // v144: the old coins in the box spend the night under frost - a pale shell on
    // each, gathering with the cold; a fresh koban wipes the patch where it lands
    const bcGeo = new THREE.CylinderGeometry(0.09, 0.09, 0.025, 14);
    for (const [bx, bz] of [[-0.55, -27.44], [-0.18, -27.72], [0.22, -27.5], [0.58, -27.68], [0.02, -27.34]]) {
      const bc = new THREE.Mesh(bcGeo, M.gold);
      bc.position.set(bx + (Math.random() - 0.5) * 0.05, 1.6, bz + (Math.random() - 0.5) * 0.05);
      bc.rotation.set((Math.random() - 0.5) * 0.22, Math.random() * 6.28, (Math.random() - 0.5) * 0.22);
      bc.castShadow = true; world.add(bc);
      const bsh = new THREE.Mesh(kobanRimeGeo, new THREE.MeshStandardMaterial({ color: 0xe8f0f6, roughness: 0.85, transparent: true, opacity: 0, emissive: 0xafc4d4, emissiveIntensity: 0.3 }));
      bsh.position.copy(bc.position); bsh.position.y += 0.002;
      bsh.rotation.copy(bc.rotation); bsh.scale.setScalar(1.06);
      world.add(bsh);
      boxFrost.push({ m: bc, rm: bsh, wipe: 0, wiping: false });
    }
  }
...
// ================= BELLS =================
const bells = [];
const bellRopeGeo = new THREE.CylinderGeometry(0.012, 0.014, 0.55, 6);
const bellRopeRimeGeo = new THREE.CylinderGeometry(0.02, 0.023, 0.5, 6); // v143: the rime sleeve
...
function offeringBell(x, note, name) {
  ...
  // v143: the pull rope with its tail - it answers the wind a half-beat behind,
  // and in the cold a rime sleeve makes it heavier: slower, stilled
  const ropeG = new THREE.Group(); ropeG.position.set(0.06, 1.32, 0.42);
  const rope = new THREE.Mesh(bellRopeGeo, M.wood); rope.position.y = -0.27; rope.castShadow = true; ropeG.add(rope);
  const tail = new THREE.Mesh(new THREE.SphereGeometry(0.026, 8, 6), M.wood); tail.position.y = -0.56; tail.castShadow = true; ropeG.add(tail);
  if (SEASON === 'snow') {
    const sleeve = new THREE.Mesh(bellRopeRimeGeo, LANTERN_SNOW); sleeve.position.y = -0.3; ropeG.add(sleeve);
    const tcap = new THREE.Mesh(new THREE.SphereGeometry(0.032, 8, 6), LANTERN_SNOW); tcap.position.y = -0.56; ropeG.add(tcap);
  }
  g.add(ropeG);
  bells.push({ x, z: -27.4, note, name, rung: false, g, bell, swingT: 0, ropeG, ropePh: Math.random() * 6.28, ropeA: 0, ropeV: 0, ropeMax: 0 });
}
...
function tossKoban() {
  ...
  setTimeout(() => {
    clink();
    boxCoins.push(coin);
    for (const c of boxFrost) { // v144: the warm coin wipes the frost where it lands
      if (!c.wiping && c.wipe < 0.5 && Math.hypot(c.m.position.x - to.x, c.m.position.z - to.z) < 0.5) c.wiping = true;
    }
...
function emaTexture(text, weather, bleed) {
  ...
  bleed = bleed || 0; // v145: the snow dawn's meltwater bleeds the ink faintly - fainter than rain's wash
  const runW = Math.max(wet, bleed * 0.6);
  ...
  lines.slice(0, 5).forEach((l, i) => {
    const ly = 125 + i * 36;
    if (runW > 0) { // the bleed: ink weeps a few pixels straight down
      x.fillStyle = 'rgba(42,33,24,' + (inkA * 0.28 * runW).toFixed(3) + ')';
      x.fillText(l, 128, ly + 3);
    }
    x.fillStyle = 'rgba(42,33,24,' + inkA.toFixed(3) + ')';
    x.fillText(l, 128, ly);
    if (runW > 0) { // drips run from under the strokes
      ...
      for (let d = 0; d < drips; d++) {
        const dx = 52 + drnd() * 152, dlen = (10 + drnd() * 26) * runW;
        x.strokeStyle = 'rgba(42,33,24,' + (0.32 * runW).toFixed(3) + ')'; x.lineWidth = 1.6;
        x.beginPath(); x.moveTo(dx, ly + 4); x.lineTo(dx + (drnd() - 0.5) * 2, ly + 4 + dlen); x.stroke();
      }
    }
  });
  if (runW > 0.4) { // rain stain pooling at the bottom edge
    const gr = x.createLinearGradient(0, 230, 0, 300);
    gr.addColorStop(0, 'rgba(70,80,88,0)'); gr.addColorStop(1, 'rgba(70,80,88,' + (0.28 * runW).toFixed(3) + ')');
...
function renderEmaRack(list, base, key) {
  list.forEach((w, i) => {
    ...
    m.userData.emaText = w; m.userData.bleed = 0; m.userData.bleedQ = 0; // v145
...
function tick() {
  ...
  if (SEASON === 'snow') {
    ...
    M.woodR.color.copy(woodRBase).lerp(woodRCold, rimeE * 0.55); // v146: the lacquer cold-dulls through the long night, warming back at first light
    M.woodR.roughness = 0.7 + 0.15 * rimeE;
    for (const sp of slotRime) sp.material.opacity = 0.75 * rimeE; // v132: the box that holds the coins takes their cold
    for (const c of boxFrost) { // v144: frost on the box coins - wiped where a koban lands, creeping back through the night
      if (c.wiping) { c.wipe = Math.min(1, c.wipe + dt / 0.7); if (c.wipe >= 1) c.wiping = false; }
      else if (c.wipe > 0) c.wipe = Math.max(0, c.wipe - dt / 900);
      c.rm.material.opacity = 0.6 * rimeE * (1 - c.wipe);
      c.rm.scale.setScalar(1.06 - 0.12 * c.wipe);
    }
    ...
    // v145: while the meltwater runs, the wish ink bleeds faintly - and the run
    // marks stay on the wood after the thaw dries
    for (const m of emaPlaques) {
      if (wetE > 0.3) m.userData.bleed = Math.min(1, (m.userData.bleed || 0) + dt * (wetE - 0.3) * 0.35);
      const q = Math.min(4, Math.floor((m.userData.bleed || 0) * 5));
      if (q !== m.userData.bleedQ && m.userData.emaText !== undefined) {
        m.userData.bleedQ = q;
        const old = m.material.map;
        const t = emaTexture(m.userData.emaText, m.userData.emaWeather, m.userData.bleed);
        m.material.map = t; m.material.emissiveMap = t; m.material.needsUpdate = true;
        if (old) old.dispose();
      }
    }
  ...
  // v142: a hard gust sets the eave icicles ticking - sparse high notes, the long
  // ones lower, each with a micro-shiver - a faint gamelan under the kairo
  if (SEASON === 'snow' && icicles.length && furinBreeze > 0.8 && T > nextIceTickT) {
    nextIceTickT = T + Math.max(0.3, 1.6 - (furinBreeze - 0.8) * 5) * (0.6 + Math.random() * 0.8);
    const ic = icicles[(Math.random() * icicles.length) | 0];
    iceTicks++;
    const f2 = 2400 - (ic.userData.len || 0.3) * 1800; // the long ones sing lower
    tone(f2, f2 * 0.92, 'sine', 0.5, 0.03);
    ic.userData.tickT = 0.35;
  }
  if (SEASON === 'snow') for (const ic of icicles) {
    if (ic.userData.tickT > 0) { ic.userData.tickT -= dt; ic.rotation.z = Math.sin(T * 40) * 0.05 * Math.max(0, ic.userData.tickT / 0.35); }
    else if (ic.rotation.z) ic.rotation.z = 0;
  }
  updateDawn(T, dt);
  // v143: the bell ropes' tails - a damped driven pendulum, so the tail answers the
  // wind a half-beat behind; rime weight slows and stills it in the cold
  for (const b of bells) {
    if (!b.ropeG) continue;
    const drive = furinBreeze * 0.5 * Math.sin(T * 1.9 + b.ropePh);
    const stiff = SEASON === 'snow' ? 2.6 : 3.6;
    const damp = SEASON === 'snow' ? 1.4 : 0.8;
    b.ropeV += ((drive - b.ropeA) * stiff - b.ropeV * damp) * dt;
    b.ropeA += b.ropeV * dt;
    b.ropeG.rotation.x = b.ropeA;
    if (Math.abs(b.ropeA) > b.ropeMax) b.ropeMax = Math.abs(b.ropeA);
  }
...
window.__wetE = v => { wetE = Math.min(1, Math.max(0, +v)); }; // v145 QA
window.__emaBleed = v => { for (const m of emaPlaques) m.userData.bleed = Math.min(1, Math.max(0, +v)); }; // v145 QA
window.__tossKoban = () => tossKoban(); // v144 QA
